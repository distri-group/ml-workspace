...
if [ ! -f "/usr/sbin/netdata"  ]; then
    ...
    cd $RESOURCES_PATH
    apt-get update
    wget https://get.netdata.cloud/kickstart.sh -O $RESOURCES_PATH/netdata-install.sh
    # Surpress output - if there is a problem remove to see logs > /dev/null
    /bin/bash $RESOURCES_PATH/netdata-install.sh --dont-wait --dont-start-it --no-updates --stable-channel --disable-telemetry --disable-cloud > /dev/null
